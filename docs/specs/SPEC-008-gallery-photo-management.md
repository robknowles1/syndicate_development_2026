...
R24: `app/javascript/controllers/gallery_sort_controller.js` is a Stimulus controller that, on `connect()`, calls `Sortable.create(this.element, { animation: 150, ghostClass: "opacity-50", onEnd: this.onEnd.bind(this) })` against the grid container it's attached to (`data-controller="gallery-sort"`). It requires no manual registration — the existing `eagerLoadControllersFrom("controllers", application)` call in `app/javascript/controllers/index.js` picks it up automatically, per the codebase's existing Stimulus convention.
