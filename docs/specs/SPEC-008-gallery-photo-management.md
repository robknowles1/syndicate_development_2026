...
R10: `GalleryPhoto` defines two named variants, one per rendered size:

- `#display_variant` returns `image.variant(resize_to_limit: [1200, 1200], saver: { quality: 80, keep: :icc })`. Used wherever the photo is displayed at full size — the public gallery's lightbox (`data-gallery-lightbox-src-param`). `resize_to_limit` preserves the source aspect ratio and never upscales, which is correct for a `object-contain` lightbox that must not crop.
- `#thumbnail_variant` returns `image.variant(resize_to_fill: [600, 600], saver: { quality: 80, keep: :icc })`. Used for every grid tile — the public gallery grid (`app/views/pages/gallery.html.erb`) and the admin grid (`app/views/admin/gallery_photos/index.html.erb`). `resize_to_fill`, not `resize_to_limit`: both grids render tiles as `aspect-square object-cover`, so a limit-fitted landscape variant is cropped to a square by the browser anyway, having first been upscaled to cover the tile. Filling to a square server-side produces the identical centre crop (`object-position` defaults to `50% 50%`, matching `resize_to_fill`'s centre gravity) at the size actually displayed, so the tile is sharper and smaller. The 600 px edge covers the largest tile any supported viewport renders — 466 CSS px on a 1920 px desktop at DPR 1, 501 device px on a 375 px phone at DPR 3.

Both variants pass `keep: :icc` to the saver. Without it, libvips copies the source's full metadata block — EXIF, XMP, IPTC and Photoshop (8bim) records — into every generated variant, measured at ~33 KB per image on this project's camera originals, paid on every variant of every photo. `keep: :icc` drops those records but retains the ICC colour profile.

`strip: true` must **not** be substituted for `keep: :icc`. It drops the ICC profile too, saving a further ~600 bytes but leaving Adobe RGB pixel data with no profile, which browsers then interpret as sRGB — measured at RMSE 3.2% / DSSIM 0.0052 against the correctly-rendered image, i.e. visibly flatter colour on a photography-led site. `spec/models/gallery_photo_spec.rb` and `spec/models/about_page_content_spec.rb` assert the ICC profile survives and the EXIF/XMP/IPTC records do not, so this substitution fails CI rather than shipping.

`keep:` requires libvips >= 8.15 (the deployed image runs 8.16). On an older libvips the option is rejected by `jpegsave` and every `.processed` call raises `Vips::Error`; because `Admin::GalleryPhotosController#create` calls `.processed` synchronously, that would surface as a 500 on upload. The existing "processes without raising" examples for both variants make a downgrade fail in CI first.

Neither variant is ever bypassed: no render call site references the original blob, so a visitor never receives a camera original.
...
AC-3: Given a `GalleryPhoto`, when `GET /gallery`, then no `<a>` element wraps any grid image (`div.grid a` is empty), and the `<button>` wrapping each thumbnail carries `data-action` including `gallery-lightbox#open` and a `data-gallery-lightbox-src-param` that is an Active Storage representation URL for that photo — never the raw original.

AC-44: Given a `GalleryPhoto`, when `GET /gallery`, then the grid `<img>`'s `src` is the `thumbnail_variant` representation URL and the wrapping button's `data-gallery-lightbox-src-param` is the `display_variant` representation URL — the two differ, so the grid never pays for full-size bytes and the lightbox never displays a 600 px crop.
...
When `GET /gallery`
Then no `<a>` tag wraps any grid image, and the wrapping `<button>` carries `data-action` including `gallery-lightbox#open` and a `data-gallery-lightbox-src-param` that is an Active Storage representation URL for that photo
Covers: R10, R12, AC-3

AT42
Given a `GalleryPhoto` with an attached image
When `GET /gallery`
Then the grid `<img>` `src` is the `thumbnail_variant` representation URL and the wrapping button's `data-gallery-lightbox-src-param` is the `display_variant` representation URL
Covers: R10, R12, AC-44
