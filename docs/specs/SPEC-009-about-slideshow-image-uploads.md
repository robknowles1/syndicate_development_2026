...
| display variant | The same named Active Storage variant options SPEC-008 defines for `GalleryPhoto` — `resize_to_limit: [1200, 1200], saver: { quality: 80, keep: :icc }` — applied here via a small `AboutPageContent` helper method, not a second variant definition. |
...
R3: `AboutPageContent` gains a helper method — e.g. `slideshow_display_variant(n)` — that returns `public_send("slideshow_image_#{n}").variant(resize_to_limit: [1200, 1200], saver: { quality: 80, keep: :icc })`. This centralizes the variant options in one place rather than repeating them at 3 view call sites, matching ADR-005 Implementation Note 4's guidance and reusing the exact same dimensions/quality SPEC-008 confirmed for `GalleryPhoto#display_variant`.
...
AC-21: At viewport widths of 375px, 390px, and 414px, the rendered admin nav produces no horizontal document overflow, keeps every item fully within the viewport, and renders each item at ≥44px tap height. At the same widths the header row keeps its title and logout button on one row, with the title unwrapped — the original failure mode, which produces no overflow and so is not caught by the overflow assertion alone.

AC-21a (correction, 2026-07-27): The widths in AC-21 were originally stated as 320/375/390px. Headless Chrome will not render below roughly 367px even under a CDP device metrics override — a narrower request is silently widened — so the 320px case was never actually exercised at 320px. Widths corrected to 375/390/414px, all of which the browser applies exactly, and `ViewportHelpers#emulate_viewport` now raises on any request below that floor rather than reporting a width it did not deliver.
...
AT21
Given admin is authenticated and the browser viewport is set to 375px, 390px, and 414px in turn
