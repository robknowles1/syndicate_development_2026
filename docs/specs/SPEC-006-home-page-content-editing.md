...
R12: `GET /admin/home_page_content` renders a form pre-filled with the current `HomePageContent` values, or with the i18n originals (the same values `restore_defaults` reads, per R18/R19) if no row exists yet — never blank. When `HomePageContent.first_or_initialize` returns a new, unsaved record (`new_record?` is `true`), the controller must assign the i18n default attributes to that in-memory record before rendering, without saving it; the DB row is only created when the admin explicitly submits the form. The form includes labeled inputs for all 4 editable fields, a `published` checkbox with hidden field to send `"false"` when unchecked, and a submit button. The form must not raise if no `HomePageContent` row exists.
...
| 2026-07-14 | Resolved R12's "(or i18n defaults / blank if no row exists)" ambiguity in favor of always pre-filling: `show` now assigns i18n default attributes to a new, unsaved `HomePageContent` record before rendering, so the form never renders blank. Discovered via manual testing of the About page equivalent (SPEC-007), which surfaced that this gap existed in both controllers but was masked here by a pre-existing seeded row. Status remains `done`; implemented directly (post-merge fix, no new review cycle). | R12 | Manual QA finding during SPEC-007 testing |
