class AdminMailer < ApplicationMailer
  def invitation(admin_user)
    ...
    @url = admin_claim_invitation_url(admin_user.generate_token_for(:invitation))
  ...
  def password_reset(admin_user)
    @admin_user = admin_user
    @url = admin_claim_password_reset_url(admin_user.generate_token_for(:password_reset))
